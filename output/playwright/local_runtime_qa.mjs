...
if (!WALLET) {
  throw new Error('Set QA_WALLET before running local_runtime_qa.mjs.');
}

if (!MNEMONIC) {
  throw new Error('Set QA_MNEMONIC before running local_runtime_qa.mjs.');
}
...
async function captureDesktopJourney() {
  ...
  try {
    await clickSemanticText(page, 'Umetni');
    await captureScreen(page, 'desktop-artist-studio');
  } catch (_) {}

  try {
    await clickSemanticText(page, 'Domov');
    await captureScreen(page, 'desktop-home-return');
  } catch (_) {}

  try {
    await clickSemanticText(page, 'Institucijsko');
    await captureScreen(page, 'desktop-institution-hub');
  } catch (_) {}

  try {
    await clickSemanticText(page, 'Web3');
    await captureScreen(page, 'desktop-web3');
  } catch (_) {}
...
async function captureMobileJourney() {
  ...
  await seedSession(page);
  await page.goto(`${APP_URL}?clear_sw=1`, { waitUntil: 'domcontentloaded' });
  await captureScreen(page, 'mobile-initial');
  ...
  try {
    await clickSemanticText(page, 'Umetni');
    await captureScreen(page, 'mobile-artist-studio');
  } catch (_) {}

  try {
    await clickSemanticText(page, 'Institucijsko');
    await captureScreen(page, 'mobile-institution-hub');
  } catch (_) {}

  try {
    await clickSemanticText(page, 'Raziskuj');
    await captureScreen(page, 'mobile-map');
  } catch (_) {}

  await browser.close();
}

await captureDesktopJourney();
