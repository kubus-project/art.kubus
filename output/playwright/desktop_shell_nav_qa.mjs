...
const USERNAME = process.env.QA_USERNAME || 'codexuiqa_seed_20260327';
const MNEMONIC = process.env.QA_MNEMONIC;

if (!WALLET) {
  throw new Error('Set QA_WALLET before running desktop_shell_nav_qa.mjs.');
}

if (!MNEMONIC) {
  throw new Error('Set QA_MNEMONIC before running desktop_shell_nav_qa.mjs.');
}
...
await capture(page, 'desktop-shell-home');

// Right rail centers based on current connected-shell layout.
await page.mouse.click(1295, 279);
await capture(page, 'desktop-shell-artist');
await page.mouse.click(28, 28);
await capture(page, 'desktop-shell-home-after-artist');

await page.mouse.click(1295, 325);
await capture(page, 'desktop-shell-institution');
await page.mouse.click(28, 28);
await capture(page, 'desktop-shell-home-after-institution');

await page.mouse.click(1295, 370);
await capture(page, 'desktop-shell-dao');
