...
if (!WALLET) {
  throw new Error('Set QA_WALLET before running mobile_nav_qa.mjs.');
}

if (!MNEMONIC) {
  throw new Error('Set QA_MNEMONIC before running mobile_nav_qa.mjs.');
}
...
await capture(page, 'mobile-map-default');

// Bottom nav targets inferred from current runtime layout.
await page.mouse.click(278, 756);
await capture(page, 'mobile-home-nav');

await page.mouse.click(214, 756);
await capture(page, 'mobile-community-nav');

await page.mouse.click(338, 756);
await capture(page, 'mobile-profile-nav');
