...
The original `SHA256SUMS` is never regenerated after the host-local overlay. A retry reuses and verifies an existing immutable SHA directory without duplicating the auth block; it never deletes or replaces that directory. The freshly uploaded artifact manifest must exactly match the existing release's original manifest or preparation fails closed. If the current cPanel policy, password file, application `.htaccess`, overlay structure, environment boundary, or separate policy record cannot be verified, preparation fails before promotion and the current live release stays selected.
