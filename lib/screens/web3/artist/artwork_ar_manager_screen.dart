      ...
      body: ListView(
        ...
        children: [
          ...
          RadioListTile<ArMarkerMode>(
            ...
            onChanged: arState.isLoading ? null : (v) => arProvider.setMarkerMode(widget.artworkId, v ?? ArMarkerMode.autoGenerated),
            title: const Text('Auto-generate'),
            subtitle: const Text('Kubus generates a QR marker for this artwork.'),
          ),
          RadioListTile<ArMarkerMode>(
            value: ArMarkerMode.userUploaded,
            groupValue: arState.markerMode,
            onChanged: arState.isLoading ? null : (v) => arProvider.setMarkerMode(widget.artworkId, v ?? ArMarkerMode.userUploaded),
            title: const Text('Upload my own'),
            subtitle: const Text('Upload a square PNG marker (512–4096px).'),
          ),
                  ...
                  child: ElevatedButton.icon(
                    onPressed: arState.isLoading
                        ...
                        : () async {
                            ...
                            await context.read<ArtworkArConfigProvider>().finalize(
                                  artworkId: widget.artworkId,
                                  walletAddress: wallet,
                                );
                            if (!mounted) return;
