...
**Screenshots:** **Evidence claim retracted 2026-08-06.** The four cited `.png` files do not exist in the repository and are not covered by `.gitignore` (only `/output/playwright/artifacts/` is ignored). `git ls-files output/playwright` returns only `.mjs`/`.json` harness files. The card behaviour is genuinely covered by `test/widgets/detail/shared_showcase_card_test.dart`; the screenshot requirement remains **unmet**. Future UI evidence must land in a tracked path and be verified with `git ls-files` before being cited here.
