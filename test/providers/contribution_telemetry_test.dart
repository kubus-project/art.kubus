...
void main() {
  ...
  Future<List<AppTelemetryEvent>> eventsOfType(String type) async {
    await pumpEventQueue();
    return (await queue.peekBatch(
      200,
    )).where((e) => e.eventType == type).toList(growable: false);
  }
  ...
  group('artwork', () {
    test(
      'submitDraft emits started then submitted on a durable artwork',
      () async {
    ...
    test(
      'a published artwork survives a telemetry subsystem that throws',
      () async {
        final l10n = await AppLocalizations.delegate.load(const Locale('en'));
  ...
  group('marker', () {
    ...
    test(
      'MarkerManagementProvider.createMarker emits one started + submitted',
      () async {
        BackendApiService().setHttpClient(
          MockClient((request) async {
  ...
  group('first-contribution milestone', () {
    test(
      'fires once for an account and again for a different account',
      () async {
        ...
        expect(milestones.map((e) => e.actorUserId).toSet(), <String>{
          userA,
          userB,
        });
      },
    ...
    test(
      'a legacy install flag cannot suppress any account milestone',
      () async {
        SharedPreferences.setMockInitialValues(<String, Object>{
